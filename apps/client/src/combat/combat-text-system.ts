...
export class CombatTextSystem {
  ...
  constructor(uiLayer: UiLayer, poolSize: number = DEFAULT_POOL_SIZE) {
    ...
    this.pool = Array.from({ length: poolSize }, (_, index) => {
      const control = this.createTextBlock(index);
  ...
  spawnDamage(mesh: AbstractMesh, damage: number, isCrit = false, entityId?: string): void {
  ...
  update(deltaTimeMs: number): void {
    ...
    for (let i = indices.length - 1; i >= 0; i -= 1) {
      ...
      instance.control.linkOffsetY = instance.baseOffsetY - instance.risePixels * t;

...
const createController = (
  onSend: (request: AbilityUseRequest) => void,
): CombatController => {
  const zoneNetwork = new ZoneConnectionManager();
  (
    zoneNetwork as unknown as { sendAbilityUse: (r: AbilityUseRequest) => void }
  ).sendAbilityUse = onSend;
  (
    zoneNetwork as unknown as { sendAbilityCancel: (r: unknown) => void }
  ).sendAbilityCancel = (_request: unknown) => {};
...
describe("CombatController", () => {
  ...
  it("clears queued ability on cancel", () => {
    ...
    (prediction as unknown as { lastRequestId?: string }).lastRequestId =
      "req-queued";

...
import type { InputManager, PointerClick } from "../input/inputManager";
...
export class GroundTargetingController implements InputHandler {
  ...
  beginTargeting(abilityId: string): void {
    ...
    if (
      !ability ||
      (ability.targetType !== "ground" && ability.directionMode !== "cursor")
    ) {
      return;
    }
  ...
  private updatePreviewTransform(): void {
    ...
    const directionYaw = this.resolveDirectionYaw(
      resolvedAbility,
      targetPoint,
      facingYaw,
    );
    ...
    const origin =
      resolvedAbility.targetType === "ground" ? targetPoint : sourcePosition;

    mesh.position.set(origin.x, origin.y + 0.05, origin.z);
    mesh.rotation.y = 0;

    this.updatePreviewRangeIndicator(
      this.isAbilityInRange(resolvedAbility, targetPoint),
    );
  ...
  private resolveDirectionMode(
    ability: AbilityDefinition,
  ): "facing" | "cursor" | "target" {
  ...
  private setAimModeHint(mode: "facing" | "cursor" | "target"): void {
    ...
    const label =
      mode === "cursor" ? "Cursor" : mode === "target" ? "Target" : "Facing";
    hint.text = `Aim: ${label}`;
    hint.isVisible = true;
  }

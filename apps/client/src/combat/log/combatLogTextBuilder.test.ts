...
import {
  buildCombatLogText,
  createCombatLogTextContext,
} from "./combatLogTextBuilder";
...
describe("CombatLogTextBuilder", () => {
  it("formats cast lifecycle text", () => {
    ...
    expect(buildCombatLogText(castStart, context)?.text).toBe(
      "Ayla begins casting Shield Bash.",
    );
  ...
  it("uses You/Your when the actor is the local player", () => {
    ...
    expect(buildCombatLogText(castStart, selfContext)?.text).toBe(
      "You begin casting Shield Bash.",
    );
    expect(buildCombatLogText(damage, selfContext)?.text).toBe(
      "You critically hit Goblin with Quick Dart for 30.",
    );
    expect(buildCombatLogText(dodged, selfContext)?.text).toBe(
      "Goblin dodges Your Quick Dart.",
    );
  });
  ...
  it("formats damage and healing effects", () => {
    ...
    expect(damageMessage?.text).toBe(
      "Ayla hits Goblin with Shield Bash for 42.",
    );
    expect(damageMessage?.parts.some((part) => part.tone === "damage")).toBe(
      true,
    );

    const critMessage = buildCombatLogText(critDamage, context);
    expect(critMessage?.text).toBe(
      "Ayla critically hits Goblin with Shield Bash for 58.",
    );
    expect(critMessage?.parts.some((part) => part.tone === "damage")).toBe(
      true,
    );

    const healingMessage = buildCombatLogText(healing, context);
    expect(healingMessage?.text).toBe("Ayla heals Ayla for 18.");
    expect(healingMessage?.parts.some((part) => part.tone === "healing")).toBe(
      true,
    );
  });
  ...
  it("formats combat state changes", () => {
    ...
    expect(buildCombatLogText(enter, context)?.text).toBe(
      "Goblin enters combat.",
    );
    expect(buildCombatLogText(exit, context)?.text).toBe(
      "Goblin leaves combat.",
    );
  });
  ...
  it("ignores non-combat events", () => {
    ...
    expect(buildCombatLogText(entry, context)).toBeNull();
  });
});

...
export class TargetingController implements InputHandler {
  ...
  private setCurrentTargetId(targetId?: string): void {
    ...
    if (localPlayer && resolved === localPlayer.getId()) {
      this.currentTargetId = undefined;
    } else {
      this.currentTargetId = resolved;
    }
  ...
  private updateIndicatorLoS(): void {
    ...
    if (inLos) {
      ...
      this.indicatorMaterial.emissiveColor =
        TargetingController.IN_LOS_EMISSIVE;

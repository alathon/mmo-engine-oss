...
import { HealthBar } from "../ui/entity/healthBar";
import { SpeechBubble } from "../ui/entity/speechBubble";
import { CastBar } from "../ui/entity/castBar";
import { applyMovementSmoothing } from "../movement/smoothing";
import { RemoteInterpolationController } from "../movement/remoteInterpolation";
import type {
  MovementState,
  NavmeshMoveDebug,
...
export class MobEntity extends Entity {
  ...
  private static readonly MODEL_MESH_OFFSET_Y = 1.0;
  ...
  constructor(name: string, scene: Scene, sync: MobState, uiLayer: UiLayer) {
    ...
    this.directionIndicator.position = new Vector3(
      0,
      MobEntity.MODEL_MESH_OFFSET_Y + 0.6,
      0.75,
    );
  ...
  override update(deltaTimeMs: number): void {
    ...
    const progress =
      isCasting && hasCastWindow
        ? (nowMs - castStart) / (castEnd - castStart)
        : 0;

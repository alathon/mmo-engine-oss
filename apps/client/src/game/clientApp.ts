...
import { ClientGameStateHandler } from "../state/clientGameStateHandler";
import { FullscreenUiController } from "../ui/fullscreenUiController";
import { createReactUiRoot, type ReactUiRoot } from "../ui/reactUiRoot";
...
export class ClientApp {
  ...
  private async transitionToIngame(
    characterId: string,
    characterName: string,
  ): Promise<void> {
    if (
      !this.scene ||
      !this.coreServices.session.loginResponse ||
      !this.reactUiRoot
    ) {
      return;
    }

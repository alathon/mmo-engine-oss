...
import { createNavMeshPolyHelper, type DebugObject } from "../zone/navcatDebug";
...
export class DebugManager {
  ...
  private movementDebugProvider?: () => MovementDebugData | null;
  ...
  private navmeshInspectProvider?: (
    point: Vector3,
  ) => NavmeshInspectData | null;
   ...
   */
  setMovementDebugProvider(provider?: () => MovementDebugData | null): void {
   ...
   */
  setPlayerInputDebugProvider(
    provider?: () => PlayerInputDebugData | null,
  ): void {
   ...
   */
  setCombatDebugProvider(provider?: () => CombatDebugData | null): void {
   ...
   */
  setNavmeshProbeProvider(provider?: () => NavmeshProbeData | null): void {
   ...
   */
  setNavmeshInspectProvider(
    provider?: (point: Vector3) => NavmeshInspectData | null,
  ): void {
  ...
  private updateMovementDebugHud(): void {
    ...
    const lines: string[] = [];
    lines.push("Movement Debug");
    lines.push(`PendingMoves: ${data.pendingMoves}`);
    lines.push(
      `ReplayΔ: ${this.formatFloat(data.lastReconcileDelta)} ` +
        `Snap: ${data.lastReconcileSnapped ? "yes" : "no"}`,
    );
    lines.push(`ReplaySeq: ${data.lastReconcileSeq}`);
    if (data.navmeshMove) {
      lines.push(
        `Move: req ${this.formatFloat(data.navmeshMove.requested)} ` +
          `act ${this.formatFloat(data.navmeshMove.actual)} ` +
          `ratio ${this.formatFloat(data.navmeshMove.ratio)}`,
      );
      lines.push(`MoveCollide: ${data.navmeshMove.collided ? "yes" : "no"}`);
      if (typeof data.navmeshMove.nodeRef === "number") {
        lines.push(`MoveNode: ${data.navmeshMove.nodeRef}`);
      }
    }
  ...
  private updatePlayerInputDebugHud(): void {
    ...
    const lines: string[] = [];
    lines.push("Input Debug");
    lines.push(`Tick: ${data.serverTick}`);
    lines.push(
    ...
    lines.push(`Budget: ${data.budgetBefore} -> ${data.budgetAfter}`);
    if (typeof data.clientPendingMoves === "number") {
      lines.push(
        `ClientPending: ${data.clientPendingMoves} ` +
          `Acked: ${data.clientLastAckedSeq ?? 0}`,
      );
    }
    if (typeof data.clientReconcileDelta === "number") {
      lines.push(
        `ReplayΔ: ${this.formatFloat(data.clientReconcileDelta)} ` +
          `Snap: ${data.clientReconcileSnapped ? "yes" : "no"}`,
      );
    }
    if (typeof data.clientReconcileSeq === "number") {
      lines.push(`ReplaySeq: ${data.clientReconcileSeq}`);
    }
  ...
  private updateCombatDebugHud(): void {
    ...
    lines.push(`Buffered: ${data.queuedAbilityId ?? "none"}`);
    if (data.target) {
      lines.push(`Target: ${data.target.id}`);
      lines.push(
        `TargetPos: ${this.formatFloat(data.target.x)} ` +
          `${this.formatFloat(data.target.y)} ` +
          `${this.formatFloat(data.target.z)}`,
      );
    } else {
    ...
    if (data.lastAck) {
      lines.push(
        `Ack: ${data.lastAck.requestId} ` +
          `${data.lastAck.accepted ? "ok" : "rej"}`,
      );
      if (!data.lastAck.accepted) {
        lines.push(`Reject: ${data.lastAck.rejectReason ?? "unknown"}`);
      }
      lines.push(`AckTick: ${data.lastAck.serverTick}`);
      lines.push(`AckServerMs: ${Math.round(data.lastAck.serverTimeMs)}`);
      lines.push(
        `Cast: ${Math.round(data.lastAck.castStartTimeMs)} -> ` +
          `${Math.round(data.lastAck.castEndTimeMs)}`,
      );
    }

    if (data.targetAggro) {
      lines.push("");
      if (data.targetAggro.length === 0) {
        lines.push("Aggro: none");
      } else {
        lines.push("Aggro:");
        for (const entry of data.targetAggro) {
          lines.push(`Aggro ${entry.id}: ${entry.percent}%`);
        }
      }
    }
  ...
  private updateNavmeshProbe(): void {
    ...
    const lines: string[] = [];
    lines.push("Navmesh Probe");
    lines.push(
      `Player: ${this.formatFloat(data.playerX)} ` +
        `${this.formatFloat(data.playerY)} ` +
        `${this.formatFloat(data.playerZ)}`,
    );
    lines.push(`OnNavmesh: ${data.isOnNavmesh ? "yes" : "no"}`);

    if (data.nearest) {
      lines.push(
        `Nearest: ${this.formatFloat(data.nearest.x)} ` +
          `${this.formatFloat(data.nearest.y)} ` +
          `${this.formatFloat(data.nearest.z)}`,
      );
      lines.push(
        `ΔXZ: ${this.formatFloat(data.nearest.distanceXZ)} ` +
          `ΔY: ${this.formatFloat(data.nearest.distanceY)}`,
      );
      lines.push(`NodeRef: ${data.nearest.nodeRef}`);
      ...
      lines.push(`Nearest: none (max ${this.formatFloat(data.maxDistance)})`);
      this.setNavmeshProbeVisible(false);
    }

    if (data.move) {
      lines.push(
        `Move: req ${this.formatFloat(data.move.requested)} ` +
          `act ${this.formatFloat(data.move.actual)} ` +
          `ratio ${this.formatFloat(data.move.ratio)}`,
      );
      lines.push(`MoveCollide: ${data.move.collided ? "yes" : "no"}`);
      if (typeof data.move.nodeRef === "number") {
        lines.push(`MoveNode: ${data.move.nodeRef}`);
      }
    }
  ...
  private setNavmeshInspectResult(result?: NavmeshInspectData | null): void {
    ...
    const lines: string[] = [];
    lines.push("Navmesh Inspect (click)");
    lines.push(
      `Click: ${this.formatFloat(result.clickX)} ` +
        `${this.formatFloat(result.clickY)} ` +
        `${this.formatFloat(result.clickZ)}`,
    );
    lines.push(`OnNavmesh: ${result.isOnNavmesh ? "yes" : "no"}`);

    if (result.nearest) {
      lines.push(
        `Nearest: ${this.formatFloat(result.nearest.x)} ` +
          `${this.formatFloat(result.nearest.y)} ` +
          `${this.formatFloat(result.nearest.z)}`,
      );
      lines.push(
        `ΔXZ: ${this.formatFloat(result.nearest.distanceXZ)} ` +
          `ΔY: ${this.formatFloat(result.nearest.distanceY)}`,
      );
      lines.push(`NodeRef: ${result.nearest.nodeRef}`);
      ...
      lines.push(`Nearest: none (max ${this.formatFloat(result.maxDistance)})`);
      this.setNavmeshInspectVisible(false);
    }

    if (result.polyInfo) {
      lines.push(
        `Tile: ${result.polyInfo.tileId} Poly: ${result.polyInfo.polyIndex}`,
      );
      lines.push(`Neighbors: ${result.polyInfo.neighbors.join(", ")}`);
      const boundaryEdges: number[] = [];
      const portalEdges: number[] = [];
      const internalEdges: number[] = [];
      for (let i = 0; i < result.polyInfo.neighbors.length; i += 1) {
        const nei = result.polyInfo.neighbors[i] ?? 0;
        if (nei === 0) {
  ...
  private updateNavmeshProbeMeshes(
    ...
    if (!this.navmeshProbeLine) {
      ...
      this.navmeshProbeLine.color = new Color3(1.0, 0.85, 0.2);
    ...
    if (!this.navmeshProbePoint) {
      ...
      mat.emissiveColor = new Color3(1.0, 0.8, 0.2);
  ...
  private updateNavmeshInspectMeshes(
    ...
    if (!this.navmeshInspectLine) {
      ...
      this.navmeshInspectLine.color = new Color3(0.2, 1.0, 0.6);
    ...
    if (!this.navmeshInspectClickPoint) {
      ...
      mat.emissiveColor = new Color3(0.2, 1.0, 0.6);
      mat.diffuseColor = new Color3(0.2, 1.0, 0.6);
    ...
    if (!this.navmeshInspectNearestPoint) {
      ...
      mat.emissiveColor = new Color3(0.2, 0.9, 1.0);
      mat.diffuseColor = new Color3(0.2, 0.9, 1.0);
  ...
  private updateNavmeshInspectPoly(navmesh: NavMesh, nodeRef: number): void {
    ...
    this.navmeshInspectPoly = createNavMeshPolyHelper(
      navmesh,
      nodeRef,
      this.scene,
    );
  ...
  private updateNavmeshInspectEdges(
    navmesh: NavMesh,
    tileId: number,
    polyIndex: number,
  ): void {
    ...
    for (let i = 0; i < vertexCount; i += 1) {
      ...
      if (nei === 0) {
        color = new Color4(1.0, 0.35, 0.35, 1);
      } else if ((nei & 0x8000) !== 0) {
        color = new Color4(1.0, 0.7, 0.2, 1);
      }
  ...
  private updateNavmeshInspectNeighborPolys(
    ...
    const nodeRefs = navmesh.nodes
      .filter((node) => node.allocated && node.tileId === tileId)
      .reduce((acc, node) => {
        acc.set(node.polyIndex, node.ref);
        return acc;
    ...
    for (const polyIndex of uniquePolyIndexes) {
      ...
      this.navmeshInspectNeighborPolys.push(
        createNavMeshPolyHelper(navmesh, ref, this.scene, [0.1, 0.6, 1.0]),
      );
    }
  }

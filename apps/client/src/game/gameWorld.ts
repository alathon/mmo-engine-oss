...
import { LocalPlayerMovementHandler } from "../movement/localPlayerMovementHandler";
...
import { getNavcatAssetUrl, getZoneDefinition } from "../zone/assetLoaders";
import { loadNavmeshFromUrl } from "../zone/navmeshLoader";
import { createNavMeshHelper, DebugObject } from "../zone/navcatDebug";
...
import { GroundTargetingController } from "../combat/groundTargetingController";
import { InputRouter } from "../input/inputRouter";
import { UiInputHandler } from "../ui/uiInputHandler";
import { buildCombatLogText, createCombatLogTextContext } from "../combat/log";
import { CombatTextSystem } from "../combat/combatTextSystem";

const RECONCILE_NUDGE_DISTANCE = 0.25;
import type { ClientSession } from "../state/clientSession";
...
export class GameWorld {
  ...
  private readonly combatLogContext = createCombatLogTextContext(
    (entityId) => this.resolveEntityName(entityId),
    (abilityId) => this.resolveAbilityName(abilityId),
    () => this.localPlayerId,
  );
  ...
  clearNavmesh(): void {
    ...
    this.players.forEach((player) => player.resetNavmeshNodeRef());
    this.npcs.forEach((npc) => npc.resetNavmeshNodeRef());
    this.updateNavmeshProbeProvider();
    this.updateNavmeshInspectProvider();
  }
  ...
  private setCameraFollowTarget(target?: TransformNode): void {
    ...
    this.camera.lockedTarget = target ?? null;
  }
  ...
  fixedTick() {
    ...
    this.players.forEach((p) => p.fixedTick(TICK_MS));
  ...
  private handlePlayerAdded(playerId: string, player: PlayerState): void {
    ...
    if (isLocal) {
      ...
      this.groundTargetingController?.setCombatController(
      ...
      this.hotbar.onSlotActivated((_slot, action) => {
        ...
        if (
          ...
          if (
            !combatController
              .getPredictionState()
              .canBufferAbility(ability, nowMs)
          ) {
            return;
          }
        ...
        const context = this.buildAutoTargetContext(action.abilityId);
        if (context === null) {
          return;
        }
  ...
  private handlePlayerUpdated(playerId: string, player: PlayerState): void {
    ...
    if (entity.isLocal) {
      ...
      if (
        this.session.characterName &&
        this.session.characterName !== entity.getName()
      ) {
        entity.setName(this.session.characterName);
      }
    } else {
  ...
  private buildAutoTargetContext(
    abilityId: string,
  ): AbilityUseContext | null | undefined {
    const ability = ABILITY_DEFINITIONS[
  ...
  private setServerPositionVisualEnabled(enabled: boolean): void {
    ...
    this.players.forEach((player) => {
      ...
      if (enabled) {
        player.serverPositionVisual.position.set(
          player.sync.x,
          player.sync.y,
          player.sync.z,
        );
    ...
    this.npcs.forEach((npc) => {
      ...
      if (enabled) {
        npc.serverPositionVisual.position.set(
          npc.sync.x,
          npc.sync.y,
          npc.sync.z,
        );
  ...
  private updateNavmeshProbeProvider(): void {
    ...
    this.debugManager.setNavmeshProbeProvider((): NavmeshProbeData | null => {
  ...
  private updatePlayerInputDebugProvider(): void {
    ...
    this.debugManager.setPlayerInputDebugProvider(
      (): PlayerInputDebugData | null => {
        const player = this.localPlayer;
  ...
  private updateCombatDebugProvider(): void {
    ...
    this.debugManager.setCombatDebugProvider((): CombatDebugData | null => {
      ...
      if (target && "combatState" in target.sync) {
        ...
        if (combatState) {
          ...
          entries.sort(
            (a, b) => b.percent - a.percent || a.id.localeCompare(b.id),
          );
          targetAggro = entries;
        }
      }
  ...
  private updateNavmeshInspectProvider(): void {
    ...
    this.debugManager.setNavmeshInspectProvider(
      (point): NavmeshInspectData | null => {
        ...
        const polyInfo =
          polyIndex >= 0
            ? {
                ...
                neighbors:
                  navmeshData.tiles[navmeshData.nodes[polyIndex].tileId]
                    ?.polys?.[navmeshData.nodes[polyIndex].polyIndex]?.neis ??
                  [],
              }
            : undefined;
  ...
  private maybeSpawnCombatText(entry: EventLogEntry): void {
    ...
    if (!damage || damage <= 0) {
      ...
      this.combatTextSystem.spawnHealing(
        healTarget.getModelMesh(),
        healing,
        effect.targetId,
      );
      return;

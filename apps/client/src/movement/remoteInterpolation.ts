...
import type { RemotePoseSample } from "./movementTypes";
...
export class RemoteMovementBuffer {
  ...
  getInterpolatedSample(
    ...
    let afterSample = this.samples[this.samples.length - 1];
    ...
    const t = Math.max(
      0,
      Math.min(1, (renderTimeMs - beforeSample.timeMs) / timeSpan),
    );
    ...
    const yaw = lerpAngle(
      beforeSample.facingYaw,
      afterSample.facingYaw,
      t,
      1000,
    );
...
export class RemoteInterpolationController {
  ...
  getRenderSample(nowMs: number): RemotePoseSample | null {
    if (!this.enabled) {
      return null;
    }

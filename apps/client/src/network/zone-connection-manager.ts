...
export class ZoneConnectionManager implements ConnectionEventEmitter {
  ...
  private playerUpdateCallback?: (playerId: string, player: PlayerState) => void;
  private playerRemoveCallback?: (playerId: string, player: PlayerState) => void;
  ...
  public onStatusUpdate(handler: (text: string, connected: boolean) => void): void {
  ...
  public onPlayerAdded(callback: (playerId: string, player: PlayerState) => void): void {
    this.playerAddCallback = callback;
  }

  public onPlayerUpdated(callback: (playerId: string, player: PlayerState) => void): void {
    this.playerUpdateCallback = callback;
  }

  public onPlayerRemoved(callback: (playerId: string, player: PlayerState) => void): void {
    this.playerRemoveCallback = callback;
  }

  public onObjectAdded(callback: (objectId: string, object: ObjState) => void): void {
    this.objectAddCallback = callback;
  }

  public onObjectUpdated(callback: (objectId: string, object: ObjState) => void): void {
    this.objectUpdateCallback = callback;
  }

  public onObjectRemoved(callback: (objectId: string, object: ObjState) => void): void {

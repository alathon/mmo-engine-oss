...
export class UiLayoutManager {
  ...
  constructor(options: UiLayoutManagerOptions = {}) {
    ...
    const initialLayouts = options.initialLayouts ?? [cloneLayout(this.baseDefaultLayout)];
    ...
    this.activeLayoutId = options.activeLayoutId ?? initialLayouts[0]?.id ?? DEFAULT_LAYOUT_ID;

    this.defaultLayout = this.layouts.get(DEFAULT_LAYOUT_ID) ?? cloneLayout(this.baseDefaultLayout);

    const activeLayout = this.layouts.get(this.activeLayoutId) ?? this.defaultLayout;
  ...
  updateWidgetLayout(widgetId: string, patch: UiWidgetLayoutPatch): void {
    ...
    if ((patch.offsetX !== undefined || patch.offsetY !== undefined) && !patch.lastViewport) {
 ...
 */
export const createUiLayoutManager = (options?: UiLayoutManagerOptions): UiLayoutManager =>
  new UiLayoutManager(options);
 ...
 */
const mergeWidgetLayout = (base: UiWidgetLayout, patch: UiWidgetLayoutPatch): UiWidgetLayout => ({
  ...base,
  ...patch,
});
 ...
 */
const areWidgetLayoutsEqual = (left: UiWidgetLayout, right: UiWidgetLayout): boolean =>

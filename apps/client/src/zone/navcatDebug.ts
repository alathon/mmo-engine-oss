...
const primitiveToBabylonJS = (
  ...
  switch (primitive.type) {
    case DebugPrimitiveType.Triangles: {
      ...
      const positions = Array.from(triPrimitive.positions);
      const indices = triPrimitive.indices
        ? Array.from(triPrimitive.indices)
        : [];
      ...
      material.emissiveColor = new Color3(1, 0, 0); // Bright red for visibility
      material.diffuseColor = new Color3(1, 0, 0);
      material.alpha = 0.5;
      material.backFaceCulling = false; // Always show both sides
...
export const createNavMeshPolyHelper = (
  ...
  nodeRef: NodeRef,
  scene: Scene,
  color: [number, number, number] = [0.2, 1.0, 0.2],
): DebugObject => {

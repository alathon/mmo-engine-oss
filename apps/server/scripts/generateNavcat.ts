...
import { generateSoloNavMesh, SoloNavMeshInput, type SoloNavMeshOptions } from "navcat/blocks";
import { ZoneDefinition, ZoneTerrainObjectDefinition } from "@mmo/shared";
import { getPositionsAndIndices } from "../src/navmesh/navcatBabylon";
...
const run = async (): Promise<void> => {
  ...
  const startingPlainsJson = await readFile(resolve(process.cwd(), "../../packages/shared/assets/zones/startingPlains.zone.json"), "utf-8");
  const startingPlainsDefinition = JSON.parse(startingPlainsJson) as ZoneDefinition;
  console.log("Zone definition", startingPlainsDefinition);
  const sceneBuilder = new BabylonSceneBuilder(startingPlainsDefinition, engine);
  const collidableMeshes = sceneBuilder.scene.meshes.filter(m => m instanceof Mesh && m.checkCollisions) as Mesh[];
  ...
  const outputDir = resolve(process.cwd(), "../../packages/shared/assets/zones");
  const outputPath = resolve(outputDir, "startingPlains.navcat.json");
  await mkdir(outputDir, { recursive: true });
  await writeFile(outputPath, JSON.stringify(result.navMesh, null, 2), "utf-8");
...
export class BabylonSceneBuilder {
  ...
  constructor(public readonly definition: ZoneDefinition, private readonly engine: AbstractEngine) {
  ...
  private createGround(): void {
    this.ground = MeshBuilder.CreateGround(
      "ground",
      { width: this.definition.sceneData.width, height: this.definition.sceneData.height },
      this.scene
    );

...
import { STATUS_DEFINITIONS } from "./status-definitions";
...
export class AbilityEngine {
  ...
  private handleMobMovement(
    ...
    if (!event.attempted && !event.moved) {
      return;
    }

    const actor = event.mob;
    if (!actor.activeCast && !actor.bufferedRequest) {
      return;
    }

    const request: AbilityCancelRequest = {
      type: "ability_cancel",
      requestId: "server-movement",
      sequence: 0,
   ...
   * Returns the ack immediately or undefined if the request was buffered.
   */
  ...
  private acceptIntoCastQueue(
    ...
    abilityState.castId = castId;

    if (gcdEndTimeMs !== undefined) {
    ...
    const castStartEvent: AbilityCastStartEvent = {
      ...
      gcdEndTimeMs: ability.isOnGcd ? gcdEndTimeMs : undefined,
      castStartTimeMs,
      castEndTimeMs,
      sourceLocation: {
        x: actor.synced.x,

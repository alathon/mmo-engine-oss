  ...
  private recomputeCache(): void {
    ...
    for (const status of this.statuses) {
      ...
      if (definition.blockedAbilityTags) {
        definition.blockedAbilityTags.forEach((tag) => blocked.add(tag));
      }
      if (definition.stateFlags) {
        definition.stateFlags.forEach((flag) => flags.add(flag));
      }
      if (definition.immunityTags) {
        definition.immunityTags.forEach((tag) => immunities.add(tag));
      }
    }

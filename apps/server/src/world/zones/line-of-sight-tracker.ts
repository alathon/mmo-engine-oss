...
export class LineOfSightTracker {
  ...
  private updatePlayer(
    ...
    const previousPos = this.lastPositions.get(player.id);
    const lastTick = this.lastUpdateTick.get(player.id) ?? -Infinity;
    const dx = previousPos ? player.synced.x - previousPos.x : Infinity;
    const dz = previousPos ? player.synced.z - previousPos.z : Infinity;
    ...
    for (const candidate of nearby) {
      ...
      if (distributionSq > this.maxRangeSq) {
        continue;
      }
  ...
  private applyVisibleTargets(player: ServerPlayer, visibleTargets: string[]): void {
    ...
    if (existing.length === visibleTargets.length) {
      let same = true;
      for (const [index, element] of existing.entries()) {
        if (element !== visibleTargets[index]) {
          same = false;
          break;
        }
      }

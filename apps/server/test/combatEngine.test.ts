...
import { AbilityEngine } from "../src/combat/abilityEngine";
import { STATUS_DEFINITIONS } from "../src/combat/statusDefinitions";
...
const createZone = (): ServerZone => {
  const zoneData = new ZoneData("combat-test-zone");
  zoneData.definition = {
    id: "combat-test-zone",
...
describe("AbilityEngine", () => {
  ...
  it("resolves circle AOE targets around the target entity", () => {
    ...
    const targets =
      acks[0]?.result?.effects[0]?.targets?.map((t) => t.targetId).sort() ?? [];
